...
module.exports = function(setup, defaultInitialState, {reducerEnhancer = null} = {}) {
  ...
  const reducer = function(state = globalInitialState, action) {
    ...
    if (action.type === RESET_STORE) {
      if (action.payload.include) {
        const reset = action.payload.include.reduce((res, key) => {
          res[key] = Object.assign({}, globalInitialState[key])
          return res
        }, {})
        return Object.assign({}, state, reset)
      } else if (action.payload.exclude) {
        const reset = Object.keys(globalInitialState).reduce((res, key) => {
          if (action.payload.exclude.indexOf(key) < 0) {
            res[key] = Object.assign({}, globalInitialState[key])
          }
          return res
        }, {})
  ...
  const wrap = function(store) {
    return Object.keys(reducers).reduce((actions, key) => {
      ...
      resetStore({exclude, include} = {}) {
        store.dispatch({type: RESET_STORE, payload: {include, exclude}})
      }
    })
  }
